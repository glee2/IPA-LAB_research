...
### 연구의 필요성
- **리튬-이온 배터리(Li-ion battery)**의 충전 용량은 충・방전을 반복함에 따라 점차 감소하며, **건강 상태(SOH: State-of-health)**은 이러한 배터리의 수명을 측정하는 지표임
...
- 배터리 제조 업체는 가속화된 실험 환경을 구성하여 배터리 건강 상태의 변화를 관찰함으로써 배터리의 내구성 평가를 진행하여 배터리 성능 검증 및 품질 개선에 활용함
  - 그러나 가속화된 실험 환경 하에서도 최소 3개월의 시간이 소요되는 등 *효율성 제고가 필수적*임
...
- 그러나 이러한 데이터 기반 모델링 방법들은 통계적 특징에 대한 의존성, 적은 데이터 샘플 수, 결과 해석의 어려움 등으로 인해 *실제로 적용되기에 한계*가 있음
...
<center> **리튬-이온 배터리 수명 열화 데이터 예시** </center>
